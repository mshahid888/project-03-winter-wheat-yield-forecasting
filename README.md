...
**Short answer:** no — not with this data. Neither Ridge regression nor Random
Forest beats the trivial baseline of "predict last year's yield". This project
documents that result carefully instead of hiding it, because knowing *why* a
simple model wins is the actual lesson.
...
the lag). Features per state-year: previous year's yield, spring (Mar–Jun of the
harvest year) temperature / precipitation / sunshine, and winter (Oct–Dec of the
previous year + Jan–Feb) temperature / precipitation. The forecast cutoff is
...
climate signal is too coarse to improve on "last year" — the regularized
linear model stays close to the dominant lag signal but cannot beat it, and
the tree ensemble overfits. The largest errors occur in 2025 in
north-western states, where an unusually sunny, dry spring coincided with
record yield jumps — exactly the kind of year-to-year regime shift a
lag-based model cannot anticipate.
...
```
project-03-agri-ml/
├── README.md  methodology.md  limitations.md  data_sources.md  requirements.txt
...
└── docs/               # interview-understanding.md
```
...
- Negative result reported as-is: neither Ridge nor Random Forest beats the
  persistence baseline; Random Forest loses clearly.
